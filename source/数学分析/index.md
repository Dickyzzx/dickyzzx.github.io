...
定理3.1.4 lim(f(x)) = A, lim(g(x))=B 

    lim(a(f(x))+b(g(x)))=aA + bB
    lim(f(x)g(x)) = AB
    lim(fx/gx)=A/B
然后是证明过程。

函数极限与数列极限的关系 (p26)
定理3.1.5 Heine定理提供了联系起函数极限与数列极限的桥梁。

$\lim_{x \to x_0} f(x)=A$ 存在的充分必要条件是: \
对任意的满足$x_m \neq x_0$,$\lim_{x_m \to \infty}=x_0$ 的数列是{$x_m$},{$f(x_m)$}收敛于A. 然后是对充分性和必要性的证明(略 23:40)